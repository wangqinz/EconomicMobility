...
scatterplotMatrix(mobility[,c(3,5,6,7,8,9,10)],main="Scatterplot matrix of Continuous Predictors")
```
...
We can observe from the pairs plots that there are some outliers shown in some panels. These outliers are the points which are far from the other points in the same panel. Two of obvious cases are the point with extreme high Mobility (ID = `r outlier_ID1` and ID =`r outlier_ID2`). We can see that in the plots of Mobility against Income, the point with the highest Mobility is far from the main tendency, which means there is one community with extreme high Mobility probability. But the Income level of the community is not so high. These two points are also shown in all panels related to Mobility.

And some outliers occur in the pairs plot of Schooling_spending and Student_teacher_ratio. Usually, a school with high spending links to a low Student_teacher_ratio. But the community with highest Student_teacher_ratio is a special case (ID = `r outlier_ID3`). For this outlier, although the School_spending is above average, the number of students is much more than the number of teachers, which lays out of the main tendency in the plot. The outlier (ID = `r outlier_ID3`) can also be found in other panels related to Student_teacher_ratio,
...
<div style= "float:left;position: relative; top: 0px; left: 0px;">
...
```{r residuals plots and QQ-plot for lm model,warning= FALSE,fig.height = 2.5,fig.cap="Above linear model"}
par(mfrow=c(1,2))
...
```{r residuals plots and QQ-plot for np model,fig.height = 2.5,fig.cap="Above kernel regression"}
par(mfrow=c(1,2))
...
```{r residuals plots and QQ-plot for gam,fig.height = 2.5,fig.cap="Above generalized additive model"}
par(mfrow=c(1,2))
...
Six figures on the left are diagnostic plots for three different models (the first row for linear regression; the second row for kernel regression and the third row for the generalized additive model). They are quite similar in some way.

For the linear regression model (two figures in the first row), the plot of residuals against fitted value show that although the means of the residuals are close to zero, the variance is not constant, that is, heteroscedasticity is not satisfied. And from the QQ-plot, there is a heavy tail on the right, indicating the residuals are not normally distributed. Also, there are several outliers shown in the plots. To summarize linear regression model does not satisfy both heteroscedasticity and normality.

The diagnostic plots of the kernel regression model (second row) are quite like the plots of the linear regression model. In residual plots, residuals do not have constant variance; in QQ-plot, there is a heavy tail on the right. The kernel regression model also does not satisfy both heteroscedasticity and normality.

As for the third model, generalized additive model, the diagnostic plots are similar to above two pairs. That may because of the dataset itself, the distribution of some predictors are not normal, which may influence some parametric tests as well as the heteroscedasticity and normality of the residuals.
...
The leave-one-out cross-validation is a model validation technique. To be more specific, we are going to fit all 16 models to 15 datasets, leaving one model out (the original model), examing how accurate a prediction model will perform in other 15 datasets. Then we will compare the mean squared error for all 16 models and determine the best model.
...
The above figure shows the prediction of mobility based on educational effectiveness (Student_teacher_ratio) and education cost (School_spending). The reason why I do not choose Graduation but the other two is in the nonparametric model, Graduation does not relate to Mobility (see last part). In this prediction figure, there are some striking horizontal patterns, indicating that compared to Student_teacher_ratio, the School_Spending is more predominant in predicting economic mobility. For example, top region shows deep red color, which means that without considering too much of Student_teacher_ratio when there is very high school expenditure, the Mobility would also be very high.
